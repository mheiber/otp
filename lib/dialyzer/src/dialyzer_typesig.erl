...
%%             PLT, PropTypes, Solvers, TolerantRecConstr) -> FunTypes
%%
%% SCC       - [{MFA}]
...
analyze_scc(SCC, NextLabel, CallGraph, CServer, Plt, PropTypes, Solvers0, TolerantRecConstr) ->
...
traverse(Tree, DefinedVars, State) ->
  ...
  case cerl:type(Tree) of
	...
	case cerl:is_literal(fold_literal_maybe_match(Tree, State1)) of
      ...
      case State2#state.tolerant_record_constr of
        true ->
          {State2, TupleType};
        false ->
          case Elements of
...
new_state(MFAs, NextLabel, CallGraph, CServer, Plt, PropTypes0, Solvers, TolerantRecConstr) ->
  List_SCC =
    [begin
  ...
  #state{callgraph = CallGraph, name_map = NameMap, next_label = NextLabel,
	 ...
	 mfas = MFAs, self_rec = SelfRec, solvers = Solvers,
         cserver = CServer, tolerant_record_constr = TolerantRecConstr}.
